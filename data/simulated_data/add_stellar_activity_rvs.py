...
def exo_stellar_activity_rvs():
    ...
    a Guassian error of 2 m/s to simulate instrumental error and other random stellar
    activity.
    ...
    Returns: 
       - List of files containing solar rvs injected into the exoplanetary rv
         systems.
    '''
    stellar_rv_files = glob.glob('generated_data/outputs/*') # Assume all solar rv curves extracted from zip to relative directory
    ...
    for i in range(len(stellar_rots)):
        ...
        new_rv = stellar_activity_rvs + exoplanet_rvs + (2 * np.random.randn(len(exoplanet_rvs)))
        dates = exo_rvs[i][0]
...
def non_exo_stellar_activity_rvs(stellar_rv_files_seen):
    ...
    for file in stellar_rv_files:
        if file not in stellar_rv_files_added_to_exoplanets:
            ...
            data = np.array([item.split() for item in rvs]).astype(float)
            stellar_activity_rvs = data.T[2] + (2 * np.random.randn(len(data.T[2])))
            phases = data.T[0]

            # Convert phases to back to date times
            rotation_period = float(file[17:-4])
