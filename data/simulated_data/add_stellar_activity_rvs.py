...
def exo_stellar_activity_rvs():
    ...
    Returns: 
       - List of files containing solar rvs injected into the exoplanetary rv
         systems.
    '''
    stellar_rv_files = glob.glob('solar_rvs/outputs/*') # Assume all solar rv curves in relative directory
    
    with open('simulated_rvs_no_solar', 'rb') as f: # Load file containing raw simulated rvs
        exo_rvs = pickle.load(f)

    with open('stellar_rot_periods', 'rb') as f: # Load file containing saved stellar rotation periods used for SOAP 2.0
    ...
    # Save simulated rvs
    with open('simulated_exo_rvs', 'wb') as f:
...
def non_exo_stellar_activity_rvs(stellar_rv_files_seen):
    ...
    # Save simulated rvs
    with open('simulated_non_exo_rvs', 'wb') as f:
